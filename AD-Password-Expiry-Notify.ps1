...
$SearchBase="FIND-IN-YOUR-AD"
$smtpServer="FIND-IN-AWS-SES-amazonaws.com"
$User = "FIND-IN-AWS-SES-SMTP-USERNAME"
$File = "C:\Users\Admin\Desktop\email\EmailPassword.txt"
...
$from = "noreply@myworld.com"
$logging = $true # Set to $false to Disable Logging
$logNonExpiring = $false
$logFile = "C:\Users\Admin\Desktop\email\PS-pwd-expiry.csv" # ie. c:\mylog.csv
$testing = $true # Set to $false to Email Users
$adminEmailAddr = "Admin@myworld.com" #multiple addr allowed but MUST be independent strings separated by comma
...
foreach ($user in $users) {
    ...
    <p>Please visit support@myworld.com to change your password.  Alternatively, on a Windows machine, you may press Ctrl-Alt-Del and select `"Change Password`".</p>

    <p>If you do not know your current/old password, create a case / send mail to  <a href='support@myworld.com'>click here to send email to support link</a>.</p>

    <p>Thank you,<br>
    # TechOps <br>
    # Admin@myworld.com<br>
     </p>
    "
    ...
    if ( ($daystoexpire -ge $negativedays) -and ($daystoexpire -lt $expireindays) -and ($daystoexpire -ne "NA") ) {
        ...
        if (($emailaddress) -ne $null) {
            if ( ($testing -eq $false) -or (($testing -eq $true) -and ($samplesSent -lt $sampleEmails)) ) {
                try {
                    Send-Mailmessage -smtpServer $smtpServer -Port 587 -Credential $cred -UseSsl -from $from -to $recipient -subject $subject -body $body -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop -ErrorVariable err
                } catch {
...
if ($logging -eq $true) {
    ...
    try {
        Send-Mailmessage -smtpServer $smtpServer -Port 587 -Credential $cred -UseSsl -from $from -to $adminEmailAddr -subject "Password Expiry Logs" -body $body -bodyasHTML -Attachments "$logFile" -priority High -Encoding $textEncoding -ErrorAction Stop -ErrorVariable err
    } catch {
